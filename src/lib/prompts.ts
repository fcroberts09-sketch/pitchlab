...
CONFIDENCE RATING GUIDE for the "confidence" field on each phase:
- "high" = the observation is directly and clearly visible in the provided frames (you can point to a specific frame)
- "medium" = the observation is consistent with what's visible but the frames don't give a complete or definitive view
- "low" = the observation is inferred or assumed; it cannot be clearly confirmed from these specific frames alone
...
/**
 * Builds the second-pass validator prompt.
 * The validator reviews the first analysis against the same frames and rates
 * each phase observation as confirmed / plausible / unsupported.
 */
export function buildValidatorPrompt(phaseSummary: string): string {
  return `You are a quality-control reviewer for a baseball pitching mechanics analysis system.

You will see video frames, then a list of phase observations generated by an AI analyzer. Your job is to independently verify whether each observation is actually supported by what is visible in the frames.

Rate each phase as:
- "confirmed"    — clearly and directly visible in the frames; you can point to a specific frame
- "plausible"    — consistent with what is visible but the frames don't provide definitive evidence
- "unsupported"  — cannot be verified from these frames; the claim may be assumed, inferred, or hallucinated

Phase observations to review:
${phaseSummary}

Return ONLY valid JSON, no markdown backticks, no preamble:
{
  "phase_validations": [
    { "name": "exact phase name", "status": "confirmed|plausible|unsupported", "note": "one sentence reason" }
  ],
  "overall_confidence": "high|medium|low",
  "flags": ["specific accuracy concern if any — or empty array"]
}`;
}
